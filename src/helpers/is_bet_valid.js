    ...
    if ("buyer_id" in auction) {
      ...
      if (
        (auction.max_bet !== null &&
          parseInt(req.body.bet) >=
            parseInt(auction.max_bet) + parseInt(auction.min_bet_step)) ||
        (auction.current_bet === null &&
      ...
      if (
        auction.max_bet !== null &&
        parseInt(auction.current_bet) >
          parseInt(auction.max_bet) - parseInt(auction.min_bet_step) &&
        parseInt(auction.current_bet) + parseInt(auction.min_bet_step) !==
          parseInt(req.body.bet)
      ) {
        res.status(400).json({
          message: "You cannot bet more than the maximum current bet + step",
        });
        return;
      }
    } else {
